...
def parse_tokens(tokens, data):
    if tokens.__len__() > 1:
        ...
        # =====================================================================================================================
            ...
            if tokens[1] == "project":
                _project_path = tokens[2]

                # connect project
                # -------------------------------------------------------------------------------------------------------------
                # syntax: connect project [existing project path]
                # -------------------------------------------------------------------------------------------------------------
                if tokens.__len__() == 3:
                    if os.path.exists((_project_path)):
                        ablauf_environment["current_project"] = _project_path
                        print(_project_path + " successfully connected")
                    else:
        ...
        # =====================================================================================================================
            ...
            if tokens[1] == "project":
                # create project
                # -------------------------------------------------------------------------------------------------------------
                # syntax: create path project [new project name] [kernel] [input_handler] [first state]
                # -------------------------------------------------------------------------------------------------------------
                if tokens.__len__() == 7:
                    _project_path = os.path.join(tokens[2],tokens[3])
                    _project_name = tokens[3]
                    _kernel = tokens[4]
                    _input_handler = tokens[5]
                    _first_state = tokens[6]
                            ...
                            if _input_handler in ['steuer', 'none_input_handler']:
                                if 0 < _first_state.__len__() < 32:
                                    ...
                                    if not os.path.exists(_project_path):
                                        os.mkdir(_project_path)

                                    if not os.path.exists(os.path.join(_project_path, "__init__.py")):
                                        # template: __init__.py
                                        template = env.get_template('init.tmp')
                                        template = template.render()
                                        save_filled_template(os.path.join(_project_path, "__init__.py"), template)
                                        # template: logging.conf
                                        template = env.get_template('logging.tmp')
                                        template = template.render()
                                        save_filled_template(os.path.join(_project_path, "logging.conf"), template)
                                        # template: controller.py
                                        template = env.get_template('empty.tmp')
                                        template = template.render()
                                        save_filled_template(os.path.join(_project_path, "controller.py"), template)

                                        # folder:   configuration
                                        os.mkdir(os.path.join(_project_path, "configuration"))
                                        ...
                                        save_filled_template(os.path.join(_project_path, "configuration", "configuration.json"), template)
                                        # template: session.json
                                        template = env.get_template('session.tmp')
                                        template = template.render()
                                        save_filled_template(os.path.join(_project_path, "configuration", "session.json"), template)
                                        # template:	translations.json
                                        template = env.get_template('translation.tmp')
                                        template = template.render()
                                        save_filled_template(os.path.join(_project_path, "configuration", "translations.json"), template)

                                        # folder:   processes
                                        os.mkdir(os.path.join(_project_path, "processes"))
                                        # template: __init__.py
                                        template = env.get_template('empty.tmp')
                                        template = template.render()
                                        save_filled_template(os.path.join(_project_path, "processes", "__init__.py"), template)

                                        # App process
                                        create_process(_project_path, "App", _first_state)

                                        # Import processes
                                        import_process(_project_path, "Input_handler", "Stema")

                                        print("project successful created in {0}".format(os.path.join(os.getcwd(), _project_path)))
                                    else:
